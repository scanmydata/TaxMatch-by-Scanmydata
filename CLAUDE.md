...
taxmatch/
  ...
  aml/                        ΔΕΟΥΣΑ ΕΠΙΜΕΛΕΙΑ (ν. 4557/2018 όπως ισχύει μετά τον ν. 5313/2026): model.py (παράγοντες Παρ. Ι/ΙΙ,
                              «ΥΠΕΡΙΣΧΥΕΙ», Μοντέλο Α/Β, προτάσεις από ΚΑΔ, συναλλαγές→παράγοντες), content.py (23 ερωτήματα ΑΑΔΕ,
                              15 βήματα, έγγραφα ανά είδος πελάτη, υποθέσεις), store.py, cases.py (καταγγελίες ν. 4990/2022 +
                              εσωτερικές αναφορές ύποπτων συναλλαγών), documents.py (PDF: Δήλωση, Ερωτηματολόγιο, Έκθεση, Ρήτρες
                              σύμβασης, Φάκελος υπόθεσης), templating.py (πρότυπα Word ×6 με πεδία ${...}/{{...}}, ανέβασμα
                              δικών του γραφείου, συμβατά και με πεδία taxis.com.gr), export.py (Excel).
                              retrieval.py: ΑΥΤΟΜΑΤΗ ΛΗΨΗ εγγράφων φακέλου (Μητρώο ΑΑΔΕ PDF, Έντυπο Ν / Ε1–Ε3, ΚΜΠΔ δικαιούχοι
                              + βεβαίωση) — port των flows του χρήστη στο recerse-engineer/runner (configs/aade-registry.js,
                              aade-fenp.js, aade-income.js, lib/kmpd-http.js). PDF -> <data>/aml_docs/<ΑΦΜ>/ + πίνακας aml_files.
                              kmpd_pdf.py: ανάγνωση PDF ΚΜΠΔ (pypdf, layout mode) → δικαιούχοι/εκπρόσωπος στον φάκελο.
                              sanctions.py: έλεγχος στην Ενοποιημένη λίστα κυρώσεων ΕΕ (λατινοποίηση ΕΛΟΤ 743 + ανεκτικό
                              ταίριασμα «σκελετού» ονόματος)· κάθε έλεγχος → `aml_screenings` (τεκμήριο).
                              gemi_browser.py: σύνδεση Γ.Ε.ΜΗ. μέσα στον ΤΟΠΙΚΟ Chrome/Edge (ξεχωριστό προφίλ, CDP με δικό
                              μας ελάχιστο WebSocket client — καμία εξάρτηση)· μετά ο χρήστης συνεχίζει μόνος.
                              GUI: gui/aml_page.py, aml_dialog.py, aml_case_dialog.py
...
`aml_profile`/`aml_assessments`/`aml_office`/`aml_register`/`aml_cases`/`aml_templates`/`aml_files`/`aml_screenings` +
κωδικοί `aml_rep_credentials`/`aml_gemi_credentials` (enc:1:) (δέουσα επιμέλεια — ΣΚΟΠΙΜΑ χωρίς FK/CASCADE: διαγραφή πελάτη ΔΕΝ σβήνει το ιστορικό, τήρηση 5ετίας άρθρο 30), `settings` (`encrypted` flag· και κατάσταση: `aade_office_status`, `llm_last_error`), `runs`, `client_credentials` (κωδικοί TAXISnet ανά πελάτη, `enc:1:`, χωριστά από το `businesses` ώστε
...
12. **Δέουσα επιμέλεια (aml/, 2026-09-23).** Βασίστηκε στο πρότυπο «Εργαλειοθήκη Συμμόρφωσης» (Taxheaven — ΔΕΝ αντιγράφηκαν
   κείμενα, μόνο η μεθοδολογία Α/Β) και σε ανάλυση του taxis.com.gr «Cloud Δέουσα επιμέλεια» (Karpodinis). **Ν. 5313/2026
   (άρθρα 88–89) άλλαξε μόνο την ΕΠΟΠΤΕΙΑ** (ΑΑΔΕ μόνο έλεγχοι, ΥΠΕΘΟ εποπτεύουσα, πρόστιμα λογιστών από ΟΕΕ) — όχι τις
   υποχρεώσεις ΔΕ. Επαληθεύτηκε από ΦΕΚ Α' 102/2026 και κοινοποίηση ΑΑΔΕ Ο 3029 ΕΞ 2026. Εγγενείς διαφορές από το taxis
   (σκόπιμες): συναλλαγές → παράγοντες Παραρτημάτων αντί για άθροισμα πόντων (στο taxis «εβδομαδιαία μετρητά σε ακίνητα» =
   15 πόντοι = Χαμηλός/Απλοποιημένη, επαληθεύτηκε ζωντανά)· ΜΙΑ κλίμακα για όλα τα είδη πελάτη (το taxis έχει άλλη ανά είδος)·
   αναφορές ύποπτων συναλλαγών στην **Αρχή (FIU)**, όχι στην ΑΑΔΕ· κατάταξη μόνο ανεβαίνει χειροκίνητα. **Ανοιχτά:** ο
   κατάλογος χωρών ΕΕ ΔΕΝ κρατιέται στατικά (σύνδεσμος)· κυρώσεις: ΜΟΝΟ λίστα ΕΕ (sanctions.py — το taxis ψάχνει ΚΑΙ OpenSanctions/OFAC non-SDN· OpenSanctions θέλει
   εμπορική άδεια)· λίστα ΠΕΠ δεν υπάρχει επίσημα (δήλωση + κουμπί Google)· ο AMLR
   (ΕΕ 2024/1624) εφαρμόζεται από 10.07.2027 — θα χρειαστεί αναθεώρηση· τα PDF δεν έχουν ελεγχθεί οπτικά σε "windows" platform·
   οι ρήτρες σύμβασης είναι ΥΠΟΔΕΙΓΜΑ προς νομικό έλεγχο.
   **Πρότυπα Word (templating.py):** τα προεπιλεγμένα .docx φτιάχνονται από κώδικα (ελάχιστο OOXML) — ΔΕΝ έχουν ανοιχτεί
   ακόμη σε πραγματικό Word. Τα πρότυπα του taxis (κατέβηκαν 2026-09-23 στο Downloads του χρήστη) γεμίζουν χωρίς να μένουν
   `${...}`· τα πεδία «πόντοι ανά συναλλαγή» της μεθόδου τους (po/plir/sixn/…) μένουν σκόπιμα κενά. Λάθη που εντοπίστηκαν
   στα έγγραφα του taxis (να μην αντιγραφούν): tipping-off «άρθρο 49/62» (σωστό 27), «άρθρο 20 = αναφορά >10.000€»
   (το 20 είναι μητρώο δικαιούχων), STR «προς ΑΑΔΕ» (σωστό: Αρχή/FIU), πεδίο ΑΦΜ γεμίζει με όνομα, λάθος ημέρα εβδομάδας.
   Και τα 6 πρότυπα του taxis (Δήλωση/Συμφωνητικό/Αξιολόγηση × φυσικά/νομικά) γεμίζουν με 0 άγνωστα πεδία
   (`TAXIS_ALIASES` + `TAXIS_BLANK` για τα σκόπιμα κενά της μεθόδου πόντων τους)· το τελευταίο («Αξιολόγηση — Φυσικά
   πρόσωπα») εξήχθη 2026-09-24 από «Ενέργειες → Επεξεργασία προτύπων γραφείου» του φακέλου πελάτη. Τα .docx του taxis
   ΔΕΝ μπαίνουν στο repo (δικά τους κείμενα) — ο χρήστης τα ανεβάζει από «Πρότυπα εγγράφων».
   **Αυτόματη λήψη εγγράφων (retrieval.py, 2026-09-24): ΔΕΝ έχει δοκιμαστεί ζωντανά από την εφαρμογή** — είναι πιστό port
   των JS flows του χρήστη (εκείνα επαληθευμένα ζωντανά στο runner), με tests μόνο σε ψεύτικο HTTP. ΜΟΝΟ ανάγνωση/προβολή
   PDF (ποτέ υποβολή/τροποποιητική). ΚΜΠΔ: κωδικοί ΝΟΜΙΜΟΥ ΕΚΠΡΟΣΩΠΟΥ (`aml_rep_credentials`, enc:1:) + ΑΦΜ εταιρείας.
   Ε1/Ε3 μόνο για έτη ≥ 2023 (τα παλιότερα θέλουν το reports/rwservlet του aade-income.js — δεν μεταφέρθηκε).
   **ΚΜΠΔ PDF (kmpd_pdf.py, 2026-09-24):** επαληθεύτηκε σε ΠΡΑΓΜΑΤΙΚΗ «Εκτύπωση δικαιούχων»/«Βεβαίωση» (δεν μπαίνουν στο
   repo — προσωπικά δεδομένα· τα tests έχουν ψεύτικο κείμενο ίδιας διάταξης). Συγχώνευση: ενημερώνει μόνο ποσοστό/έλεγχο,
   ποτέ ΠΕΠ/ταυτότητα/χώρα του χρήστη· κατάσταση δικαιούχων μόνο αν ήταν κενή → «εκκρεμεί ταυτοποίηση».
   **Κυρώσεις (sanctions.py):** επαληθεύτηκε στην πραγματική λίστα ΕΕ (6.241 οντότητες, αρχείο 22/09/2026): «ΒΛΑΝΤΙΜΙΡ
   ΠΟΥΤΙΝ», «ΣΕΡΓΚΕΪ ΛΑΒΡΟΦ», «ΣΑΝΤΑΜ ΧΟΥΣΕΪΝ» βρίσκονται· κοινά ελληνικά ονόματα δεν δίνουν ψευδώς θετικά. Αποτέλεσμα =
   «πιθανή ταύτιση» προς έλεγχο — ΠΟΤΕ αυτόματο «ΥΠΕΡΙΣΧΥΕΙ».
   **Γ.Ε.ΜΗ. (gemi_browser.py): ΔΕΝ έχει δοκιμαστεί ζωντανά** (δεν υπήρχαν κωδικοί). Ο CDP/WebSocket client επαληθεύτηκε
   με πραγματικό headless Chrome (about:blank). Η σύνδεση είναι το `gemiLogin` του runner εκτελεσμένο same-origin μέσα
   στη σελίδα του businessportal. Προφίλ: `<data>/browser/gemi`· η θύρα debugging (127.0.0.1, τυχαία) μένει ανοιχτή όσο
   τρέχει εκείνος ο browser.
   **Έλεγχος taxis (2026-09-24) — λειτουργίες τους που δεν έχουμε ακόμη (λίστα στο `TODO.md`):** αποστολή ερωτηματολογίου/επικαιροποίησης με
