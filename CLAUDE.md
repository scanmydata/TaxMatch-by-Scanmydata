...
```bash
.venv/Scripts/python.exe -m pytest                 # 110 tests, ~5s, χωρίς δίκτυο
...
taxmatch/
  ...
  business_profiles/          import_excel, lookup_business_portal (ΓΕΜΗ), lookup_aade (TAXISnet), vat_profile, service
...
`settings` (`encrypted` flag), `runs`. Νέα αλλαγή schema = νέο στοιχείο στο `MIGRATIONS` (ποτέ επεξεργασία υπάρχοντος).
...
1. **Credentials: γραφείου ή ανά πελάτη;** Τώρα υλοποιείται ΜΟΝΟ ένας λογαριασμός TAXISnet γραφείου (Ρυθμίσεις). Η Φάση 6 (myDATA/ΕΡΓΑΝΗ/TAXISnet ανά πελάτη) θα αποθηκεύει κωδικούς πολλών πελατών — απαιτεί συνειδητή απόφαση πριν χτιστεί.
...
6. **Πηγές:** forin.gr (χωρίς RSS → HTML scraper, Φάση 2 του MIGRATION_PLAN), taxlive.gr (άγνωστο feed), καθώς και `soft_art.xml`/`soft_lawl.xml` του taxheaven δεν έχουν ενταχθεί. Το ίδιο θέμα σε δύο πηγές δίνει διπλά άρθρα (μόνο URL dedup).
7. **Φάση 7 (ML):** τα labeled δεδομένα συσσωρεύονται ήδη στο `matches.user_feedback`. Δεν έχει γραφτεί ML.
8. `data/kad_catalog.csv` (στατικός πίνακας ΚΑΔ 2008) δεν υπάρχει — τα ΚΑΔ εμφανίζονται με την περιγραφή που δίνει το ΓΕΜΗ.
