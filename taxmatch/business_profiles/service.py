...
def lookup_and_store(conn: sqlite3.Connection, afm: str, session: Optional[requests.Session] = None,
    ...
    """Ανάκτηση από ΓΕΜΗ (όνομα/νομική μορφή/ΚΑΔ) και ΑΑΔΕ (ΔΟΥ/ΦΠΑ/κατηγορία βιβλίων). Κάθε πηγή είναι προαιρετική.
    Επιστρέφει {'status', 'errors': [...], 'sources': [...]} και ενημερώνει τη γραμμή."""
    if not get(conn, afm):
        raise KeyError(afm)
    errors: list[str] = []
    sources: list[str] = []
    ...
    if user and pwd:
        ...
        try:
            ...
            if a.get("ok"):
                raw["aade"] = {"kind": a.get("kind"), "all_tags": a.get("all_tags")}
                vat = interpret_vat_profile(a.get("all_tags") or {})
                fields["doy"] = a.get("doy") or ""
                fields.setdefault("name", a.get("name") or "")
                if a.get("address"):
                    fields.setdefault("address", a["address"])
                fields["_vat"] = vat
                sources.append("ΑΑΔΕ")
            else:
    ...
    status = "failed" if not sources else ("ok" if (have_kads and (fields.get("name") or row["name"])) else "partial")
    ...
    conn.execute(f"UPDATE businesses SET {', '.join(sets)} WHERE afm=?", args)
    return {"status": status, "errors": errors, "sources": sources}
